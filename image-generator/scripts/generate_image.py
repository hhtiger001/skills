...
# 生成图片的命名（用户选择后填入）
OUTPUT_NAME = "tiger_xuanwu_001"

# 参考图片路径（用户选择后填入，填相对于 REFERENCE_DIR 的文件名）
IMAGE_PATHS = ["../reference_images/tiger_portrait_001.jpg", "../reference_images/tiger_portrait_002.jpg"]
...
# 图片存放在 skill 脚本目录下的 reference_images
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
REFERENCE_DIR = os.path.join(SCRIPT_DIR, "gen")
...
def get_full_path(filename):
    """获取完整路径：如果已经是完整路径则直接返回，否则拼接 REFERENCE_DIR"""
...
def main():
    ...
    # 保存原始JSON
    with open(os.path.join(output_dir, "response.json"), "w", encoding="utf8") as f:
    ...
    for i, src in enumerate(saved_files):
        if OUTPUT_NAME:
            ...
            existing = [f for f in os.listdir(REFERENCE_DIR) if f.startswith("gen_")]
            idx = len(existing) + 1
    ...
    if generated_filenames:
        try:
            ...
            latest_path = os.path.join(REFERENCE_DIR, latest_filename)
