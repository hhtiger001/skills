...
def save_images_from_content(content, output_dir):
    ...
    for m in re.finditer(base64_pattern, content):
        ...
        ext = ".png"
        filename = f"{OUTPUT_NAME}{ext}" if OUTPUT_NAME else f"image_{img_index}.png"
...
def main():
    ...
    if generated_filenames:
        try:
            latest_filename = generated_filenames[0]
            latest_path = get_output_path(latest_filename)
